...
new_addpts = "WayneCoPts_20230517"
...
#    where_clause = "STREETNAME IS NOT NULL AND STREET IS NULL"
    fields = ['Pre', 'Name', 'Dir', 'Type', 'Street']
...
def check_nearby_roads(working, streets, gdb):
    ...
    addpt_fields = ['OBJECTID', 'HouseNumbe', 'Street', 'Notes']
...
def logic_checks(row):
    ...
    goodstreet = False
    goodnum = False
    add_num = ''.join(i for i in row['HouseNumbe'] if i.isdigit())
    
    if add_num.isdigit():
        if row['Street'] == row['STREET']:
            goodstreet = True
            if (int(row['HouseNumbe'].split()[0]) >= row['L_F_ADD'] and int(row['HouseNumbe'].split()[0]) <= row['L_T_ADD']) or (
                    int(row['HouseNumbe'].split()[0]) >= row['R_F_ADD'] and int(row['HouseNumbe'].split()[0]) <= row['R_T_ADD']):
                goodnum = True
        ...
        if row['Notes'] == 'no near st: possible typo predir or sufdir error':
            if (int(row['HouseNumbe'].replace('-', ' ').split()[0]) >= row['L_F_ADD'] and int(row['HouseNumbe'].replace('-', ' ').split()[0]) <= row['L_T_ADD']) or (
                    int(row['HouseNumbe'].replace('-', ' ').split()[0]) >= row['R_F_ADD'] and int(row['HouseNumbe'].replace('-', ' ').split()[0]) <= row['R_T_ADD']):
                goodnum = True
