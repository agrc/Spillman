...
def combine_addpts(points):
    ...
    point_fields = ["FID", "OBJECTID", "SHAPE", "ADDRES", "CITYCD", "OLD_AD", "GRID", "STREET", "PARCEL", "NUMBER", "AUDIT", "OWNER"]
    ...
    for fld in combined_fields:
        ...
        if "PROPERTY_O" in fld.name.upper() or "OWNER" in fld.name.upper()[:5]:
            arcpy.AlterField_management(combined_points, fld.name, "OWNER")
        if "AUDIT" in fld.name.upper()[:5]:
