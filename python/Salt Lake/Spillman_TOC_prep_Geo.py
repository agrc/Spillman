...
utm_db = r"C:\E911\TOC\TOC_Geovalidation_UTM.gdb"
wgs84_db = r"C:\E911\TOC\TOC_Geovalidation_WGS84.gdb"
...
def create_address_pts_CAD(addpts):
    where_clause = "FullAdd NOT LIKE '% UNIT%' AND FullAdd NOT LIKE '% TRLR%' AND FullAdd NOT LIKE '% APT%' AND" \
                   ...
                   " FullAdd NOT LIKE '% HNGR%' AND FullAdd NOT LIKE '% OFC%'"
    # Need to make layer from feature class here
...
def export_shapefiles_select_fields_rename(fc, folder, field_list, outname, query):
    print("Output folder is: {}".format(folder))
    print("Exporting {} to shapefile with the following fields:".format(fc))
    for field in field_list:
        print(field)
    ...
    for index, field in enumerate(field_list):
        ...
        d["fm{}".format(index)].outputField = d["fname{}".format(index)]
        fms.addFieldMap(d["fm{}".format(index)])                                # add FieldMap to FieldMappings object
    arcpy.FeatureClassToFeatureClass_conversion(fc, folder, outname, where_clause=query, field_mapping=fms)
...
WGS84_files_to_delete = ["TOC_Streets", "CityCodes", "CommonNames", "RoadFeatures",
                         "MiscZones", "EMSZones", "EMSAreas", "FireZones", "FireAreas",
                         "LawZones", "LawAreas", "TOC_Streets_CAD", "Municipalities",
                         "tbzones"]
...
FCs_to_project = ["TOC_Streets", "CityCodes", "CommonNames", "RoadFeatures",
                         "MiscZones", "EMSZones", "EMSAreas", "FireZones", "FireAreas",
                         "LawZones", "LawAreas", "TOC_Streets_CAD", "Municipalities"]
...
commplc_fields = ["CommonName", "FullAddr"]
street_fields = ["L_F_ADD", "L_T_ADD", "R_F_ADD", "R_T_ADD", "ZIPLEFT", "ZIPRIGHT", "STREET", "LCITYCD", "RCITYCD"]
ezone_fields = ["Description", "ZoneID"]
earea_fields = ["Name", "ReportingArea"]
fzone_fields = ["NAME", "ZONEID"]
farea_fields = ["NAME", "AREAID"]
lzone_fields = ["NAME", "ZONEID"]
larea_fields = ["NAME", "AREAID"]
mz_fields = ["NAME", "ZONEID"]
citycd_fields = ["NAME", "CityCD"]
muni_fields = ["SHORTDESC", "POPLASTCENSUS"]

# Keep separate or combine as one shapefile? Add exits
ramp_fields = ["RFName", "RFType", "CITYCD", "BEGNUMB", "STREET"]
milepost_fields = ["RFName", "RFType", "CITYCD", "BEGNUMB", "STREET"]
...
export_shapefiles_select_fields_rename("RoadFeatures", out_folder_spillman, milepost_fields, "TOC_Milemarkers", mp_query)
ramp_query = """RFType = 'RAMP SIGN'"""
export_shapefiles_select_fields_rename("RoadFeatures", out_folder_spillman, ramp_fields, "RampSigns", ramp_query)


# Shapefiles that aren't needed for TOC PSAP, but are available:
# export_shapefiles_select_fields_rename("CommonNames", out_folder_spillman, commplc_fields, 'TOC_CommonPlaces_FC', '')
# export_shapefiles_select_fields_rename("EMSZones", out_folder_spillman, ezone_fields, 'TOC_EZ_Zones', '')
# export_shapefiles_select_fields_rename("EMSAreas", out_folder_spillman, earea_fields, 'TOC_EZ_Areas', '')
# export_shapefiles_select_fields_rename("FireAreas", out_folder_spillman, farea_fields, 'TOC_Fire_Areas', '')
# export_shapefiles_select_fields_rename("LawAreas", out_folder_spillman, larea_fields, 'TOC_Law_Areas', '')
# export_shapefiles_select_fields_rename("Municipalities", out_folder_spillman, muni_fields, 'TOC_Municipalities', '')
